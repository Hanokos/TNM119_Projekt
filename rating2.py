...
if os.path.exists('Model3.pkl') and os.path.exists('vectorizer3.pkl'):
    ...
    model = XGBClassifier(random_state=42, objective='multi:softmax', num_class=5)
...
# ... (rest of the script remains unchanged until Step 18)

# Step 18: Visualize Local Explanation for the Last Data Point
# Initialize SHAP explainer
explainer = shap.TreeExplainer(best_model3)
shap_values = explainer.shap_values(X_test)  # Get SHAP values for the test set

# Get the predicted class for the last data point
predicted_class = y_pred[-1]

# Get the SHAP values for the predicted class of the last data point
shap_values_last = shap_values[predicted_class][-1]  # SHAP values for the predicted class
feature_names = vectorizer3.get_feature_names_out()  # Feature names

# Create a DataFrame for the SHAP values and feature names
shap_df = pd.DataFrame({
    'Feature': feature_names,
    'SHAP Value': shap_values_last
})

# Sort by absolute SHAP values to highlight the most important features
shap_df['Absolute SHAP Value'] = shap_df['SHAP Value'].abs()
shap_df = shap_df.sort_values(by='Absolute SHAP Value', ascending=False).head(20)  # Top 20 features

# Plot the local explanation as a bar plot
plt.figure(figsize=(10, 6))
plt.barh(shap_df['Feature'], shap_df['SHAP Value'], color='lightgreen')
plt.xlabel('SHAP Value (Impact on Prediction)')
plt.ylabel('Feature')
plt.title(f'Local Explanation for the Last Data Point (Predicted Class: {predicted_class})')
plt.gca().invert_yaxis()  # Invert Y-axis to show the most important feature at the top

# Overwrite if the file already exists
if os.path.exists('local_explanation.png'):
    os.remove('local_explanation.png')
plt.savefig('local_explanation.png')  # Save the local explanation plot as an image
plt.show()

# Step 19: Print completion message
print("Visualizations saved as 'accuracy_pie_chart.png', 'feature_importance.png', and 'local_explanation.png'.")
